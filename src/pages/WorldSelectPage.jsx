...
function Stars({ n }) {
  return (
    <div style={{ display:'flex', gap:'2px' }}>
      {[1,2,3].map(i => (
        <span key={i} style={{ fontSize:'0.75rem', opacity: i<=n ? 1 : 0.2 }}>⭐</span>
      ))}
    </div>
  );
}
...
export default function WorldSelectPage() {
  ...
  useEffect(() => {
    stopNarration(); // clear any carry-over
    timerRef.current = setTimeout(() => narrateText(
    ...
    <PageShell phase="play">
      <div style={{ maxWidth:'680px', width:'100%', display:'flex', flexDirection:'column', gap:'0.65rem' }}>
        ...
        <div style={{ textAlign:'center' }}>
          <h2 style={{ fontFamily:'Baloo 2', fontWeight:900, fontSize:'1.4rem', color:'var(--gold)' }}>
            🎮 Choose Your World!
          </h2>
        ...
        <div style={{ display:'flex', gap:'0.5rem', justifyContent:'center', flexWrap:'wrap' }}>
          {[['⭐','Stars',`${totalStars}/30`],['🔥','Streak',`${state.streak}`],
            ['✅','Correct',`${totalCorrect}/100`],['⚡','XP',`${state.xp}`]].map(([ic,lb,vl]) => (
            <div key={lb} style={{
              background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)',
              borderRadius:'0.75rem', padding:'0.38rem 0.7rem', textAlign:'center', minWidth:'66px',
            }}>
          ...
          {worlds.map(w => {
            ...
            return (
              <div
                ...
                onClick={() => !locked && handlePlay(w.id)}
              ...
              >
                ...
                <span style={{ fontFamily:'Baloo 2', fontWeight:800, fontSize:'0.68rem', color:'#fff', lineHeight:1.2 }}>
                  {w.name}
                </span>
                <span style={{ fontFamily:'Nunito', fontWeight:600, fontSize:'0.58rem', color:'rgba(255,255,255,0.45)' }}>
                  Q{(w.id-1)*10+1}–{w.id*10}
                </span>
        ...
        {allDone && (
          <button className="btn-gold" style={{ width:'100%' }}
            onClick={() => { stopNarration(); navigate('/reflect'); }}>
            🏆 See Your Results!
          </button>
        )}
      </div>
